...
class ContentGenerator:
    ...
    async def generate(
        ...
        if is_movie:
            data = await self._recover_movie_generation_length(
        ...
        if not is_movie and not self._is_valid_content(data):
        ...
        if is_movie and any(len(line.split()) > 29 for line in lines):
            raise RuntimeError("Movie script exceeds 29 words per line after repair.")
