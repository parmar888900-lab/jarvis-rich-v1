...
class ContentGenerator:
    ...
    async def _recover_movie_generation_length(
        ...
        *,
        ...
        """
        Recover undersized grounded movie-facts narration.

        Recovery is bounded and fail-closed:
        - exactly four narration lines
        - 75-110 narration words
        - no narration line over 29 words
        - evidence IDs must come from the current research packet
        - claim/evidence validation must pass
        - evidence labels must never leak into narration
        """
        ...
        # -------------------------------------------------
        # Build the ONLY legal evidence-ID vocabulary from
        # the actual research supplied to this generation.
        # -------------------------------------------------
        ...
        for evidence_id in evidence_matches:
            ...
            if (
                ...
                allowed_evidence_ids.append(
                    normalized
                )

        # Some research representations use E1: rather
        # than [E1]. Support that form without inventing IDs.
        ...
        if not allowed_evidence_ids:
            logger.warning(
                ...
                "research contains no parseable evidence IDs."
            )
            return None
...
You need approximately {missing_words} additional words
if the current script is short.

Do not merely repeat existing wording.

Add only useful explanation directly supported by the
...
- verify every evidence ID comes from ALLOWED EVIDENCE IDS
...
script_lines must contain exactly four strings.

evidence_ids must contain exactly four lists,
one list corresponding to each narration line.

CRITICAL EVIDENCE CONSTRAINT:

The ONLY evidence IDs you may cite are:

{allowed_evidence_text}

Do NOT create a new evidence ID.
Do NOT guess an evidence ID.
Do NOT use an ID merely because it appeared in an example.
...
5. Preserve natural spoken vertical-video narration.

6. Line 1 must preserve the exact curiosity-driving
premise and create an information gap.

7. Line 2 provides only necessary context.

8. Line 3 contains the strongest concrete supported
mechanism, example, action, contrast, number,
or consequence available in the evidence.

9. Line 4 resolves the opening information gap with
a specific supported payoff.

10. Every line must add new information.

11. Never paste raw research blocks into narration.

12. Never place [E1], [E2], or ANY evidence label
inside script_lines.

13. evidence_ids remain separate from narration.

14. Every factual claim must be supported by the
supplied grounded evidence.

15. Cite ONLY IDs from ALLOWED EVIDENCE IDS.

16. Do not invent filmmaking, production,
behind-the-scenes, historical, numerical,
causal, or technical claims.

17. Do not add a generic call to action.

18. Do not add filler merely to reach word count.

19. Count the four narration lines before responding.

20. Count total narration words before responding.
...
""",
            ...
            raw_evidence_ids = recovered.get(
                "evidence_ids",
                [],
            )
            ...
            for line_ids in raw_evidence_ids:
            ...
            if unknown_ids:
                ...
                # Do not carry invented IDs into the next
                # prompt. Preserve only IDs proven to exist.
                recovered[
                    "evidence_ids"
                ] = sanitized_evidence_ids
            ...
            if any(
                not line_ids
                for line_ids
                in sanitized_evidence_ids
            ):
                logger.warning(
                    "Movie length recovery attempt %s "
                    "left one or more narration lines "
